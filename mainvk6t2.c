...
// Funktioprototyyppi
void eteenpainpropagointi(float input_data[]);
...
void main(void)
{
    ...
    // Tulostetaan mittausarvot
    printk("x = %d, y = %d, z = %d\n", m.x, m.y, m.z);

    // Käytetään mittausarvoja syötteenä
    float input_data[3] = { (float)m.x, (float)m.y, (float)m.z };
    
     // Lasketaan eteenpäinpropagointi
    eteenpainpropagointi(input_data);
